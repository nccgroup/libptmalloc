...
class pt_helper:
    ...
    def bin_at(self, m, i):
        ...
        return int(
            gdb.parse_and_eval(
                "&((struct malloc_state *) 0x%x).bins[%d]"
                % (m.address, int((i - 1) * 2))
            ).cast(gdb.lookup_type(cast_type))
            - offsetof_fd
        )
...
class pt_chunk(pt_structure):
    ...
    def __init__(self, pt, addr=None, mem=None, size=None, inferior=None, inuse=None):
        ...
        if is_gdb:
            ...
            try:
                mem2 = self.inferior.read_memory(
                    nextchunk_addr + self.pt.SIZE_SZ, self.pt.SIZE_SZ
                )
        ...
        if self.hdr_size == self.pt.FREE_FASTCHUNK_HDR_SZ:
            if self.address != None:
                ...
                if self.inferior != None:
                    if self.pt.SIZE_SZ == 4:
                        mem = self.inferior.read_memory(
                            self.address, self.pt.FREE_FASTCHUNK_HDR_SZ
                        )
                    elif self.pt.SIZE_SZ == 8:
                        mem = self.inferior.read_memory(
                            self.address, self.pt.FREE_FASTCHUNK_HDR_SZ
        ...
        # smallbin freed follows
            ...
            if self.address != None:
                ...
                if self.inferior != None:
                    if self.pt.SIZE_SZ == 4:
                        mem = self.inferior.read_memory(
                            self.address, self.pt.FREE_HDR_SZ
                        )
                    elif self.pt.SIZE_SZ == 8:
                        mem = self.inferior.read_memory(
                            self.address, self.pt.FREE_HDR_SZ
            ...
            if self.pt.SIZE_SZ == 4:
                (self.fd, self.bk) = struct.unpack_from(
                    "<II", mem, self.pt.INUSE_HDR_SZ
                )
            elif self.pt.SIZE_SZ == 8:
                (self.fd, self.bk) = struct.unpack_from(
                    "<QQ", mem, self.pt.INUSE_HDR_SZ
                )
        # largebin freed freed follows
            ...
            if self.address != None:
                ...
                if self.inferior != None:
                    if self.pt.SIZE_SZ == 4:
                        mem = self.inferior.read_memory(
                            self.address, self.pt.FREE_LARGE_HDR_SZ
                        )
                    elif self.pt.SIZE_SZ == 8:
                        mem = self.inferior.read_memory(
                            self.address, self.pt.FREE_LARGE_HDR_SZ
            ...
            if self.pt.SIZE_SZ == 4:
                (
                    ...
                    self.fd_nextsize,
                    self.bk_nextsize,
                ) = struct.unpack_from("<IIII", mem, self.pt.INUSE_HDR_SZ)
            elif self.pt.SIZE_SZ == 8:
                (
                    self.fd,
                    self.bk,
                    self.fd_nextsize,
                    self.bk_nextsize,
                ) = struct.unpack_from("<QQQQ", mem, self.pt.INUSE_HDR_SZ)
    ...
    def __str__(self):
        ...
        # also share the fd/bk, we could refactor code here?
            ...
            if (
                self.pt.prev_inuse(self)
                or self.pt.chunk_is_mmapped(self)
                or self.pt.chunk_non_main_arena(self)
            ):
                ret += " ("
...
class pt_save_state(pt_structure):
    ...
    def __init__(self, pt, addr=None, mem=None, inferior=None):
        ...
        if self.pt.SIZE_SZ == 4:
            ...
            (self.magic, self.version) = struct.unpack_from("<QQ", mem, 0x0)
            ...
            (
                ...
                self.arena_max,
                self.narenas,
            ) = struct.unpack_from("<QQQQQQQQQIIQQQQQQQ", mem, sbrk_base_off)

    def __str__(self):
        string = []
        string.append(
            "%s%lx%s%s%lx%s%lx%s%lx%s%lx"
            % (
...
if is_gdb:
    ...
    class ptstats(ptcmd):
        ...
        def invoke(self, arg, from_tty):
            ...
            while 1:
                ...
                # account for top
                avail = self.pt.chunksize(
                    pt_chunk(self.pt, self.pt.top(ar_ptr), inuse=True)
                )
                nblocks = 1
                ...
                for i in range(1, self.pt.NBINS):
                    ...
                    p = pt_chunk(
                        self.pt.first(pt_chunk(self.pt, b, inuse=False)), inuse=False
                    )
    ...
    class ptcallback(ptcmd):
        ...
        def help(self):
            ...
            self.pt.logmsg(
                " register <name> <module> use a global function <name> as callback from <module>"
            )
        ...
        def invoke(self, arg, from_tty):
            ...
            if arg.find("enable") != -1:
                ...
                if self.pt.ptchunk_callback:
                    ...
                    self.pt.ptchunk_callback == None and self.pt.ptchunk_callback_cached
                ):
    ...
    class ptchunk(ptcmd):
        ...
        def help(self):
            self.pt.logmsg(
                "usage: ptchunk [-v] [-f] [-x] [-p offset] [-c <count>] [-s <val] [--depth <depth>] <addr>"
            )
    ...
    class ptarena(ptcmd):
        ...
        def help(self):
            ...
            self.pt.logmsg(
                " <addr> a ptmalloc mstate struct. Optional with cached mstate"
            )
            self.pt.logmsg(" -v     use verbose output (multiples for more verbosity)")
    ...
    class ptarenaof(ptcmd):
        ...
        def help(self):
            ...
            self.logmsg(
                " <infile.txt> a filename for a file containing one ptmalloc chunk header address per line"
            )
            return
